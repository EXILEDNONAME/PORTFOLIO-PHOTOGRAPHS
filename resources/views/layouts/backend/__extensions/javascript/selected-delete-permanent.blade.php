<script>
  $('#selected-delete-permanent').on('click', function(e) {
    ...
    Swal.fire({ text: "{{ __('default.notification.confirm.selected-delete-permanent') }}", icon: "warning", showCancelButton: true, confirmButtonText: '{{ __("default.label.yes") }}', cancelButtonText: '{{ __("default.label.no") }}', reverseButtons: false }).then(function(result) {
      if (result.value) {
        $.ajax({
