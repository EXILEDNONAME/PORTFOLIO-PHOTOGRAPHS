...
class CrudMigrationCommand extends GeneratorCommand
{
  ...
  */
  protected function getPath($name)
  {
    ...
    return database_path('/migrations/') . $datePrefix . '_create_main_' . $name . '.php';
  }
  ...
  */
  protected function buildClass($name)
  {
    ...
    $className = 'Create' . str_replace(' ', '', ucwords(str_replace('_', ' ', $tableName)));
  ...
  $schemaUp =
  "Schema::create('main_" . $tableName . "', function (Blueprint \$table) {
      \$table->increments('" . $primaryKey . "');
    $schemaFields\$table->integer('active')->default(1); \n" . $tabIndent . $tabIndent . $tabIndent .
      "\$table->integer('status')->default(1); \n" . $tabIndent . $tabIndent . $tabIndent .
      "\$table->timestamps();\n" . $tabIndent . $tabIndent . $tabIndent .
      "\$table->softDeletes();\n" . $tabIndent . $tabIndent . "});";

    $schemaDown = "Schema::dropIfExists('main_" . $tableName . "');";
