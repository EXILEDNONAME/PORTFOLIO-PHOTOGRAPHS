...
class CrudCommand extends Command
{
  ...
  */
  public function handle()
  {
    ...
    $this->call('crud:requeststore', ['name' => $controllerNamespace . $modelName . '\StoreRequest', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
    $this->call('crud:requestupdate', ['name' => $controllerNamespace . $modelName . '\UpdateRequest', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations]);
    ...
    $this->call('crud:model', ['name' => $modelNamespace . $modelName, '--fillable' => $fillable, '--table' => $tableName, '--pk' => $primaryKey, '--relationships' => $relationships, '--soft-deletes' => $softDeletes, '--foreign-keys' => $foreignKeys]);
  ...
  * @return  array
  */
  // Rote::resource('" . $this->routeName . "', '" . $this->controller . "');
  protected function addRoutes() {
    $name = $this->argument('name');
    $routeControllerName = Str::singular($name) . 'Controller';

    return ["Route::group([
  'as' => 'dashboard.main.$this->routeName.',
